...
const GOAL_KEYS = [
  ...
  "fatLoss",
  "muscleDefinition",
  "balancedRecomposition",
] as const;

type GoalKey = (typeof GOAL_KEYS)[number];

/** English values sent to the API (prompt expects English). */
const GOAL_API_VALUES: Record<GoalKey, string> = {
...
const TIMEFRAME_API = "12 weeks";

export default function TransformationAiPage() {
  const { messages: m } = useLanguage();
  const t = m.transformationAi;

  const [file, setFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [goal, setGoal] = useState<GoalKey>("leanToned");
  ...
  const resultLabel = useMemo(() => {
    return result ? t.preview.transformationPreview : t.preview.aiPreviewLabel;
  }, [result, t.preview.transformationPreview, t.preview.aiPreviewLabel]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.append("goal", GOAL_API_VALUES[goal]);
      formData.append("timeframe", TIMEFRAME_API);
      ...
      if (!response.ok || !payload.imageBase64) {
        throw new Error(payload.error || t.errors.generateFailed);
      }
  ...
  return (
    <>
      <Navbar />
      <main className="relative overflow-hidden bg-[#fff8ee] pt-28 text-[#111111]">
            ...
            <div>
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-3">
                <div className="border border-[#E5E7EB] bg-white/80 p-4 shadow-sm">
                  <p className="text-xs font-semibold text-[#6B7280]">{t.stats.planLength}</p>
                  <p className="mt-2 text-lg font-semibold">{t.stats.weeks12}</p>
                </div>
                <div className="border border-[#E5E7EB] bg-white/80 p-4 shadow-sm">
                  <p className="text-xs font-semibold text-[#6B7280]">{t.stats.photoType}</p>
                  <p className="mt-2 text-lg font-semibold">{t.stats.frontFacing}</p>
                </div>
                <div className="border border-[#E5E7EB] bg-white/80 p-4 shadow-sm">
                  <p className="text-xs font-semibold text-[#6B7280]">{t.stats.output}</p>
                  <p className="mt-2 text-lg font-semibold">{t.stats.aiPreview}</p>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="mt-8 space-y-5 border border-[#E5E7EB] bg-white/90 p-6 shadow-lg">
                ...
                <label className="flex flex-col gap-2 text-sm font-semibold text-[#111111]">
                  ...
                  <div className="border border-dashed border-[#CBD5F5] bg-[#F8FAFF] px-3 py-4 text-sm">
                    ...
                    />
                    <p className="mt-2 text-xs text-[#6B7280]">{t.form.uploadHint}</p>
                  </div>
                </label>
                ...
                <p className="text-xs text-[#6B7280]">{t.form.disclaimer}</p>
              </form>
            </div>

            <div className="space-y-6">
              <div className="border border-[#E5E7EB] bg-white p-4 shadow-sm">
                <p className="text-xs font-semibold text-[#6B7280]">{t.preview.original}</p>
                <div className="mt-3 flex h-64 items-center justify-center bg-[#F3F4F6]">
                  {previewUrl ? (
                    ...
                    <span className="text-sm text-[#9CA3AF]">{t.preview.uploadToPreview}</span>
                  )}
                </div>
              </div>
              ...
              <div className="border border-[#E5E7EB] bg-white p-4 shadow-sm">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold text-[#6B7280]">{resultLabel}</p>
                  {result && (
                    <a
                ...
                <div className="mt-3 flex h-64 items-center justify-center bg-[#F3F4F6]">
                  {result ? (
                    ...
                    <span className="text-sm text-[#9CA3AF]">{t.preview.willAppearHere}</span>
                  )}
                </div>
              </div>
              <div className="border border-[#E5E7EB] bg-white p-4 shadow-sm">
                <p className="text-xs font-semibold text-[#6B7280]">{t.tips.title}</p>
